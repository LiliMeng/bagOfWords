...
class cv_vocabulary_tree
{
    ...
    int nLabel_;   // number of labels, every image has one label
...
public:
    ...
    void buildTree(const cv::Mat & data,
                   ...
                   const cv_vocabulary_tree_parameter & para, vector<int> & Ni);

    // query the distribution of multiple feature, each vector in a row
    void query(const cv::Mat & features, vector<float> & distribution, vector<int> Ni) const;
...
private:
    void buildTree(cv_vocabulary_tree_node * node,
                   const cv::Mat & data,
                   const vector<unsigned int> & labels,
                   const cv_vocabulary_tree_parameter & para, int depth, vector<int> & Ni);
