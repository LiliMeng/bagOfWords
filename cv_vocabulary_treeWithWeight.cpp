...
ofstream fout2("/home/lili/workspace/SLAM/vocabTree/leafnodeSize.txt");
                                   ...
                                   const cv_vocabulary_tree_parameter & para)
{
    ...
    cout<<"data.rows is "<<data.rows<<endl;
    cout<<"data.cols is "<<data.cols<<endl;
    cout<<"labels.size() is: "<<labels.size()<<endl;
    cout<<"para.nLabel_ is "<<para.nLabel_<<endl;
                                   ...
                                   const cv_vocabulary_tree_parameter & para, int depth)
{
    ...
    if(depth > para.max_depth_ || data.rows <= para.min_leaf_node_)
    {
       ...
       for(int i=0; i<labels.size(); i++)
        ...
        for(int i=0; i<labels.size(); i++)
        {
            //cout<<"labels["<<i<<"] "<<labels[i]<<endl; //有很多labels相同　

            node->histgram_[labels[i]]+=1.0;   //不是在这里加weight　因为这里计算的是每个node的
   ...
   // printf("acutal k is %d\n", K);
    ...
    for (int i = 0; i<clusters.rows; i++)
    {
        ...
        splitted_datas[idx].push_back(data.row(i));
